...
// Upload image binary to S3
const uploadImageBinary = (inData) => {
  s3Client.putObject(inData, function (err, data) {
    if (err) {
      console.log(err);
      throw err;
    } else {
      console.log(`File successfully uploaded to S3 with ID ${inData.Key}!`);
    }
  });
};
...
const getFile = async (fileKey) => {
  ...
  try {
    ...
    var file = fs.createWriteStream(
      "C:/Users/Joseph/Documents/CS554_Good_Nodels/backend/data/cat.jpg"
    );
